...
let get_input= In_channel.with_open_text "inputs/day1.input" In_channel.input_lines;;
...
let part1 lines =
  let (_dial, num_zeros) = List.fold_left
...
let part2 lines =
  let (_dial, num_zeros) = List.fold_left
    (fun (dial, num_zeros) x ->
        ...
        let crossings = if offset >= 0 
          then new_pos /% 100 - dial /% 100
          else (dial - 1) /% 100 - (new_pos - 1) /% 100 in
